...
class FriendTestCase(APITestCase):
  ...
  def test_friend(self):
    ...
    # johnnysilverhand follow back
    johnwick = Author.objects.get(displayName="johnwick").id
    johnnysilverhand = Author.objects.get(displayName="johnnysilverhand").id
    payload = {
      "actor": johnnysilverhand.replace("http://testserver/author/",""),
      "object": johnwick
    }
    header = {
      "HTTP_AUTHORIZATION": "Basic " + base64.b64encode("johnnysilverhand:johnnysilverhand123456".encode()).decode()
    }
    response = self.client.put(johnwick+'/followers/'+johnnysilverhand.replace("http://testserver/author/","")+"/", payload, **header)
    self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    self.assertEqual(len(Follower.objects.get(owner=johnwick).items), 1)