...
from presentation.models import Author, Post, Comment, Likes
...
class LikeTestCase(APITestCase):
  ...
  def test_post_like(self):
    ...
    # johnwick likes johnnysilverhand's comment
    johnnysilverhand = Author.objects.get(displayName="johnnysilverhand").id
    johnwick = Author.objects.get(displayName="johnwick").id
    # should be no like yet
    self.assertEqual(Likes.objects.count(), 0)
    payload = {
      "authorID": johnnysilverhand,
      "type":"Like",
      "postID": response_json['id'],
      "actor": johnwick,
      "object": json.loads(response.content.decode('utf-8'))['id'],
      "summary":"I like your comment!",
      "context":response_json['id']
    }
    header = {
      "HTTP_AUTHORIZATION": "Basic " + base64.b64encode("johnwick:johnwick123456".encode()).decode()
    }
    response = self.client.post(response_json['id']+'/likes/', payload, **header)
    self.assertEqual(response.status_code, status.HTTP_200_OK)
    # should be one like exist
  ...
  def test_comment_like(self):
    ...
    self.assertFalse(post.unlisted)
    self.assertEqual(post.visibility, "PUBLIC")
    # johnnysilverhand like johnwick's post
    johnnysilverhand = Author.objects.get(displayName="johnnysilverhand").id
    johnwick = Author.objects.get(displayName="johnwick").id
    response_json = json.loads(response.content.decode('utf-8'))
    payload = {
      "actor": johnnysilverhand,
      "author": johnnysilverhand,
      "authorID": johnwick,
      "context": response_json['id'],
      "object": response_json['id'],
      "postID": response_json['id'],
